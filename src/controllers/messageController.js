...
async function handleGlobalActions(req, res) {
  ...
  if (["subscribe", "abonner", "اشتراك"].includes(lowerCaseMessage)) {
    ...
    const existing = await db
      ...
      .findOne({ whatsappId: From });

    if (!existing || !existing.isSubscribed) {
      // new user OR unsubscribed user → subscribe now
  ...
  if (
    ["unsubscribe", "désabonner", "الغاء الاشتراك"].includes(lowerCaseMessage)
  ) {
    const db = getDB();
    await db
      .collection("subscribers")
      .updateOne({ whatsappId: From }, { $set: { isSubscribed: false } });
    await sendMessage(
      From,
      messages.languages[lang].general.unsubscribeConfirm
    );
    return true; // Action was handled
  }

  return false; // the user didn't type subscribe/unsubscribe & no audio was detected so pass directly to the flow handlers
}
